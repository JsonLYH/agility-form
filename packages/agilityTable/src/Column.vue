<template>
  ...
  <el-table-column
    v-bind="item"
    :align="item.align || $agility.agilityTable.align"
      ...
      <template v-if="item.type === 'click'">
        <span
          v-if="
            ...
            formatText(item, scope.row) === item.fieldEmptyTxt ||
            formatText(item, scope.row) === '--'
          "
...
export default {
  ...
  methods: {
    ...
    formatText(item, row) {
      ...
      if (item.filter) {
        if (item.filter == 'money') {
          ...
          if (money == 0) return 0;
          return money || item.fieldEmptyTxt || this.$agility.fieldEmptyTxt;
        } else if (item.filter == 'date') {
          return (
            formatDate(text, 'yyyy-MM-dd ') || item.fieldEmptyTxt || this.$agility.fieldEmptyTxt
          );
        } else if (item.filter == 'datetime') {
          return formatDate(text) || item.fieldEmptyTxt || this.$agility.fieldEmptyTxt;
        }
      }
      if (text === null || text == undefined || text === '')
        return item.fieldEmptyTxt || this.$agility.fieldEmptyTxt;
      if (Number(text) == 0) return text;
      if (text) return text;
      return this.$agility.fieldEmptyTxt;
    },
    /**
     * Link文本内容格式化
     * @param {item} 当前列对象
    ...
    formatLink(item, row, link) {
      ...
      if (text) return text;
      if (item.fieldEmptyTxt) return item.fieldEmptyTxt;
      return this.$agility.fieldEmptyTxt;
    },
  },
};
