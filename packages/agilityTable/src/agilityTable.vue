...
export default {
  ...
  computed: {
    ...
    config() {
       ...
       */
      let { toolbar, align, stripe, border, showPagination, pageSize, tableEmptyText } =
        this.$agility.agilityTable;
      ...
      if (typeof json.showPagination == 'undefined') json.showPagination = showPagination;
      if (!json.pagination) json.pagination = {};
      if (!json.pagination.pageSize) json.pagination.pageSize = pageSize;
      if (!json.emptyText) json.tableEmptyText = tableEmptyText;
      return {
        ...json,
        // 表格上面标题
